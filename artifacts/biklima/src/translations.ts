export type Lang = "ar" | "en";

export const T = {
  ar: {
    nav: {
      structure: "البرامج التدريبية",
      wisdom: "مقتطفات",
      workbooks: "الكراسات",
      testimonials: "آراء",
      gallery: "معرضنا",
      videos: "فيديوهات",
      events: "الدورات الحضورية",
      courses: "الدورات",
      cta: "ابدأ رحلتك",
      mobileCta: "ابدأ رحلتك الآن",
    },
    hero: {
      badge: "منهج متكامل في فنّ الخطابة والتأثير",
      h1a: "لديك ما تقوله —",
      h1b: "وبكلمة ستُسمَع.",
      sub: "كثيرون يخسرون فرصهم لأن أفكارهم تتوه بين العقل والكلام. في المقابلة، الاجتماع، المنصة، والحياة — بكلمة هي الحل لمن أسكته التردد ولم يُسمع بعد.",
      ctaPrimary: "ابدأ التعلم الآن",
      ctaSecondary: "اكتشف البرامج التدريبية",
      trustStrip: {
        trainees: "أكثر من ٨٠٠ متدرب",
        countries: "٧ دول",
        rating: "تقييم ٤٫٩ / ٥",
        years: "خبرة +٦ سنوات",
      },
      institutions: {
        label: "شارك بكلمة أو مؤسسها في برامج مع",
        list: "Qatar Foundation · UAE Ministry of Economy · Aston University · Breakthrough T1D · TEDx",
      },
      compare: {
        heading: "قارن بين البرامج",
        sub: "أربعة برامج · مسار واحد · اختر ما يناسبك ومسارك.",
        bestPick: "الأنسب للبدء",
        colProgram: "البرنامج",
        colAudience: "الفئة المستهدفة",
        colDuration: "المدة",
        colPrice: "السعر المسجَّل",
        colOutcome: "النتيجة المتوقّعة",
        colPrereq: "المتطلب السابق",
        none: "لا يوجد",
        viewDetails: "تفاصيل البرنامج",
        viewAll: "اطّلع على كل البرامج",
        priceSchools: "للمدارس فقط",
      },
      imageQuotes: [
        { text: "لا تخف من صوتك... خف من اليوم الذي تُقرر فيه ألا تتكلم.", author: "صهيب الخوالدة" },
        { text: "الكلمة الصادقة تصل أبعد من ألف خطاب مزخرف.", author: "صهيب الخوالدة" },
        { text: "ليس المهم أن تتكلم... المهم أن يبقى أثر كلامك.", author: "صهيب الخوالدة" },
        { text: "صوتك هو أقوى أدوات التغيير التي تملكها.", author: "صهيب الخوالدة" },
        { text: "الثقة لا تُولد... تُبنى كلمة بعد كلمة.", author: "صهيب الخوالدة" },
        { text: "من يملك الكلمة يملك التأثير.", author: "صهيب الخوالدة" },
      ],
    },
    trainerBio: {
      heading: "عن المؤلف والمدرب",
      name: "صهيب الخوالدة",
      title: "مستشار · باحث دكتوراه · متحدث TEDx · مؤسس بكلمة",
      bio: "مستشار يعمل مع مجموعة من المؤسسات التنموية والخيرية والحكومية في المنطقة، من بينها Qatar Foundation، ووزارة الاقتصاد والسياحة في دولة الإمارات، بالإضافة إلى مؤسسة التمويل Breakthrough T1D وغيرهم، حيث يساهم في تصميم البرامج، وبناء الشراكات، ودعم مبادرات التنمية والابتكار.\n\nمحاضر في عدد من الجامعات، منها American University في الإمارات، وجامعة Aston في المملكة المتحدة في الاقتصاد الرقمي وريادة الأعمال. وهو باحث دكتوراه في Aston University (UK)، ويحمل درجة الماجستير في الإدارة من University of Birmingham (UK).\n\nمتحدث TEDx ومؤلف سلسلة كراسات بكلمة التدريبية. يقدّم برامج تدريبية في فن الخطابة والإلقاء، ومهارات القيادة المهنية، وتمويل المشاريع كامتداد لخبرته في العمل والتأثير المؤسسي. قدّم عشرات البرامج والخطابات في الأردن والعالم العربي، ودرّب مئات المتدربين من مختلف الأعمار والخلفيات.",
    },
    author: {
      sectionTitle: "رسالة إلى كل معلم وخطيب ومتدرب",
      cards: [
        { to: "إلى المعلم والمربّي", text: "أنت لا تُعلّم مادة — أنت تُشكّل أصواتاً. كل كلمة تقولها أمام طالبك تُبقي أثراً في طريقة حديثه مع العالم. برنامجنا وُلد من إيمان عميق بأن أجمل ما يمكن أن تمنحه لطالبك هو أن يُؤمن بأن صوته يُغيّر." },
        { to: "إلى الخطيب والمتحدث", text: "كلامك ليس مجرد أداء — هو حضور. والحضور لا يأتي من التقنية وحدها، بل من الاتصال بما تؤمن به حقاً. بكلمة لا تُعلّمك كيف تتكلم بل تُعيدك إلى سبب كلامك، وتمنحك الأدوات لتوصل روحك قبل صوتك." },
        { to: "إلى المتدرب المبتدئ", text: "لا يهم من أين تبدأ. يهم أنك تبدأ. ٧٠٪ من الناس يعانون من رهاب التحدث، لكن الأثر الحقيقي يصنعه الـ ٣٠٪ الذين قرروا أن يتعلموا. أنت هنا — وهذا هو القرار الأول والأصعب." },
      ],
    },
    why: {
      heading: "لماذا بكلمة؟",
      sub: "المشكلة ليست أن ليس لديك ما تقوله — المشكلة أن ما تقوله لا يصل كما تريد.",
      items: [
        { title: "لأن صمتك في اللحظة الحاسمة له ثمن", desc: "خسارة وظيفة لأن المقابلة لم تُقنع. فكرة عظيمة ضاعت لأن أحداً لم يسمعها. شراكة لم تُعقد لأن الكلمات لم تكن في محلّها. هذه ليست مشكلة تعبير — هي خسارة حقيقية يمكن تجنّبها." },
        { title: "لأن المشكلة ليست في الأفكار", desc: "معظم من يعانون من ضعف التواصل لديهم أفكار عميقة وحقيقية. المشكلة في الفجوة بين ما يُفكّرون فيه وما يصل للآخرين. بكلمة يجسّر هذه الفجوة بمنهجية تعمل على اللفظ والذهن والأداء معاً." },
        { title: "لأن الثقة مهارة، لا موهبة", desc: "٧٠٪ ممن يصمتون لم يُولدوا خائفين — بل تعلّموا الخوف في مكان ما. نعمل على الجذور قبل الأداء. ما بُني يمكن إعادة بنائه — وهذا تحديداً ما نفعله." },
      ],
      consultIntro: {
        heading: "احجز جلستك الاستشارية المجانية",
        what: "جلسة فردية مع صهيب الخوالدة مدّتها ٢٠ دقيقة — تُحدّد فيها وضعك الحالي والمسار الأنسب لك.",
        audiences: [
          { icon: "🎓", label: "طلاب وخريجون", desc: "تميّز في المقابلات وابنِ شخصيتك المهنية" },
          { icon: "💼", label: "مهنيون وموظفون", desc: "قُد الاجتماعات واجعل صوتك يُسمع في المؤسسة" },
          { icon: "🏫", label: "معلمون ومدرّبون", desc: "أسلوب إلقاء يُلهم ويُقنع ويُؤثّر في الفصل" },
          { icon: "👨‍👩‍👧", label: "آباء وأولياء أمور", desc: "ابنِ ثقة طفلك بالتعبير من سنٍّ مبكرة" },
        ],
      },
    },
    structure: {
      heading: "البرامج التدريبية",
      sub: "دورات مسجّلة، مباشرة عبر Zoom، أو مزيج بينهما — أما المواعيد الوجاهية فتجدها في قسم مستقل أدناه.",
      coreBadge: "الدورة الأساسية المحورية",
      coreSubtitle: "صناعة التأثير وفن الإلقاء والخطابة",
      hoursUnit: "ساعة",
      sessionsUnit: "جلسة",
      viewDetails: "اكتشف البرنامج",
      viewProgramPage: "اكتشف البرنامج",
      prereqLabels: { tot: "متطلب: دورة المتحدث المؤثر", teachers: "يُستحسن: دورة المتحدث المؤثر", children: "عبر خريجي برنامج المعلمين" },
      recordedLabel: "مسجّلة",
      recordedDesc: "ادرس في أي وقت يناسبك",
      zoomLabel: "مباشر عبر Zoom",
      blendedLabel: "مسجّل + Zoom",
      deliveryOptions: "مسجّل · Zoom · مزيج بينهما",
      recordedPriceLabel: "سعر الدورة المسجّلة",
      inPersonLabel: "وجاهي",
      privateLabel: "تدريب خاص ١:١",
      privateDesc: "حسب الطلب — الموعد والسعر يُحددان لاحقاً",
      contactForPrivate: "تواصل معنا",
      upcomingEventsHeading: "الدورات الحضورية",
      upcomingEventsSub: "مواعيد التدريب الحضوري القادمة — اطّلع على التفاصيل وسجّل مباشرة من هنا",
      noUpcomingEvents: "لا توجد دورات وجاهية معلنة حالياً — تابعنا لمعرفة المواعيد الجديدة",
      countriesHeading: "الدول التي نقدّم فيها دوراتنا الوجاهية",
      countries: { uae: "الإمارات", saudi: "السعودية", jordan: "الأردن", uk: "المملكة المتحدة" },
      registerNow: "سجّل الآن",
      spotsLeft: "مقعد متبقي",
      enrollNow: "احجز مقعدك",
      priceUnit: "د.أ",
      rating: "٤.٩",
      startHelpHeading: "تحتاج إلى استشارة قبل البدء؟",
      startHelpSub: "أخبرنا بهدفك واحتياجك — سنساعدك في اختيار البرنامج الأنسب لك ولمسارك.",
      startHelpBtn: "ساعدني في اختيار البرنامج",
      startHelpContact: "تواصل معنا",
    },
    wisdom: {
      badge: "مقتطفات من داخل الكراسات",
      heading: "فلسفة بكلمة",
      sub: "ما تجده في كراساتنا ليس تمارين فحسب — بل رؤية تربوية تؤمن بأن الكلمة أداة بناء إنسان.",
    },
    workbooks: {
      heading: "الكراسات التدريبية",
      sub: "كراسات تدريبية مصممة بعناية — يمكن اقتناؤها بشكل مستقل أو ضمن البرنامج.",
      pricesIn: "الأسعار بـ",
      priceLabel: "السعر",
      orderBtn: "شراء الكراسة",
      orderBtnKit: "شراء الحقيبة",
      companion: "الفئة المستهدفة",
      previewTitle: "معاينة الصفحات",
      previewPage: "صفحة",
      samplePdfBtn: "تحميل عينة PDF",
      samplePdfNote: "حمّل نسخة تجريبية مجانية من الكراسة للاطلاع على المحتوى",
      orderTitle: "طلب شراء الكراسة",
      quantityLabel: "عدد النسخ",
      formatLabel: "الصيغة",
      formatPdf: "نسخة رقمية (PDF)",
      formatPrint: "نسخة مطبوعة",
      deliveryLabel: "عنوان التوصيل",
      deliveryPlaceholder: "المدينة، الشارع، رقم البناية...",
      deliveryNote: "السعر لا يشمل رسوم التوصيل — سيتم تأكيد تكلفة التوصيل لاحقاً.",
      totalLabel: "المجموع",
      submitOrder: "تأكيد طلب الشراء",
      submittingOrder: "جاري الإرسال...",
      orderSuccess: "تم استلام طلبك بنجاح! سنتواصل معك قريباً.",
      nameLabel: "الاسم الكامل",
      namePlaceholder: "اسمك الكامل",
      phoneLabel: "رقم الهاتف",
      emailLabel: "البريد الإلكتروني",
    },
    testimonials: { heading: "آراء عملائنا" },
    testimonialsSection: {
      eyebrow: "آراء عملائنا",
      heading: "كلماتٌ من قلوب من سبقوك",
      sub: "أصواتٌ حقيقية من متدربين ومدربين وأولياء أمور — هؤلاء بدأوا رحلتهم معنا، فلِمَ لا تكون التالي؟",
    },
    stats: {
      eyebrow: "بكلمة بالأرقام",
      heading: "أثرٌ يتسع كل يوم",
      sub: "أرقامٌ تروي قصة آلاف اللحظات التي تحوّل فيها الصمت إلى صوت.",
      trainees: "متدرّب ومتدرّبة",
      countries: "دول حول العالم",
      programs: "برامج تدريبية",
      trainers: "مدرّب معتمد",
    },
    beforeAfter: {
      eyebrow: "قبل وبعد",
      heading: "تحوّل حقيقي يبدأ من أول جلسة",
      sub: "ليس مجرد دورة — بل رحلة تُعيد تشكيل علاقتك بصوتك وأفكارك وحضورك.",
      beforeLabel: "قبل بكلمة",
      beforeHeading: "أنت قبل أن تبدأ",
      beforeItems: [
        "أفكارك واضحة في رأسك لكنها تتبعثر عند الكلام.",
        "ترتجف يداك وصوتك أمام أي جمهور — حتى الصغير.",
        "تخسر فرصاً مهنية بسبب مقابلات لم تُقنع.",
        "تتجنّب التعبير عن رأيك في الاجتماعات والنقاشات.",
        "تظنّ أن الخطابة موهبة لا تُكتسب.",
      ],
      transformLabel: "التحول",
      afterLabel: "بعد بكلمة",
      afterHeading: "أنت بعد البرنامج",
      afterItems: [
        "تنطق فكرتك في ثوانٍ بوضوح وثقة.",
        "تدخل أي غرفة وأنت تعرف أن صوتك يستحق أن يُسمع.",
        "تُقنع في المقابلات وتقود الاجتماعات بحضور.",
        "تُلقي خطابات تترك أثراً يبقى بعدها.",
        "تمتلك أدوات الخطابة كمهارة قابلة للبناء.",
      ],
    },
    journey: {
      eyebrow: "رحلة الطالب",
      heading: "كيف تبدأ رحلتك مع بكلمة؟",
      sub: "ست خطوات واضحة من لحظة اهتمامك حتى استلامك للشهادة المعتمدة.",
      steps: [
        { title: "سجّل اهتمامك", desc: "املأ نموذج التسجيل أو احجز استشارة مجانية." },
        { title: "تختار برنامجك", desc: "راجع تفاصيل البرامج واختر المسار الذي تريد التسجيل فيه." },
        { title: "تبدأ التدريب", desc: "حضور جلسات وجاهية أو عن بُعد مع المدرّب." },
        { title: "تطبّق عملياً", desc: "تمارين أداء حقيقية وتغذية راجعة فردية." },
        { title: "تحصل على تقييم", desc: "تقييم احترافي لخطابك مع توصيات للنمو." },
        { title: "تستلم الشهادة", desc: "شهادة معتمدة من بكلمة تشهد على إنجازك." },
      ],
    },
    journeyCta: {
      heading: "ابدأ رحلتك مع بكلمة اليوم",
      sub: "خطوة واحدة تفصلك عن أن يصبح صوتك أداتك الأقوى.",
      button: "ابدأ رحلتك",
    },
    faq: {
      heading: "الأسئلة الشائعة",
      sub: "كل ما تحتاج لمعرفته عن برامجنا ومساراتها.",
      items: [
        { q: "ما الدورة التي يجب أن أبدأ بها؟", a: "الدورة الأساسية \"المتحدث المؤثر\" هي المدخل الطبيعي. إذا كنت تريد الوصول للاعتماد كمدرب فستحتاج إتمامها أولاً. أما إذا كنت معلماً أو ولي أمر فيمكنك الالتحاق ببرنامجنا المتخصص مباشرةً." },
        { q: "ما الفرق بين التدريب الجماعي والتدريب الخاص؟", a: "التدريب الجماعي يتيح لك المشاركة ضمن مجموعة سواء عن بعد أو وجاهياً. أما التدريب الخاص ١:١ فهو تجربة مكثفة ومخصصة تماماً لك (وجاهي أو عن بعد)، مع وتيرة أسرع وتغذية راجعة فورية ومعمّقة." },
        { q: "هل يشترط إتمام الدورة الأساسية قبل برنامج المعلمين وأولياء الأمور؟", a: "لا يشترط ذلك. لكن إتمام الدورة الأساسية يعزز الاستفادة القصوى ويعطيك تجربة المتدرب قبل أن تكون المرشد." },
        { q: "كيف يصل برنامج الأطفال للمدارس؟", a: "برنامج \"المتحدث الصغير\" يُقدَّم للمدارس حصراً عبر خريجي برنامج المعلمين وأولياء الأمور المعتمدين من بكلمة." },
        { q: "هل يمكن شراء الكراسة دون الاشتراك في البرنامج؟", a: "نعم! يمكنك شراء أي كراسة بشكل مستقل. الكراسات مصممة لتكون مفيدة بمفردها، لكن الاستفادة القصوى تكون مع البرنامج المرافق." },
        { q: "هل البرامج حضورية أم إلكترونية؟", a: "نقدم البرامج بعدة صيغ: تدريب جماعي عن بعد، تدريب جماعي وجاهي، وتدريب خاص ١:١ وجاهي أو عن بعد." },
        { q: "هل تقدمون تدريباً مخصصاً للمؤسسات التعليمية؟", a: "نعم! نقدم برامج تأهيل مخصصة للمؤسسات التعليمية لتدريب معلميها وإدماج مادة الخطابة ضمن برامجها غير المنهجية. تواصل معنا عبر نموذج المؤسسات في صفحة التسجيل." },
        { q: "كم مدة البرنامج التدريبي؟", a: "تختلف المدة حسب البرنامج: الدورة الأساسية ٢٧ ساعة (١٤ جلسة)، المدرب المعتمد ٤٠ ساعة (٢٠ جلسة)، المعلمين وأولياء الأمور ٢١ ساعة (١١ جلسة)، والمتحدث الصغير ١٨ ساعة (٩ جلسات)." },
        { q: "هل هناك خصومات متوفرة؟", a: "نعم، نقدم خصومات للمجموعات وللتسجيل المبكر. تواصل معنا مباشرةً وسنوضح لك الأسعار والعروض المتاحة." },
        { q: "هل يمكنني تقديم رابط لخطاب سابق لي أثناء التسجيل؟", a: "بالطبع! نشجعك على مشاركة رابط يوتيوب لأي خطاب أو عرض سابق. هذا يساعدنا في فهم مستواك الحالي وتقديم تجربة تدريبية أكثر تخصيصاً." },
      ],
    },
    enroll: {
      heading: "للأفراد والمؤسسات",
      sub: "اطلب عرضًا مخصصًا لمؤسستك أو مجموعتك. للأفراد: استخدم زر «ابدأ التعلم الآن» في الأعلى للتسجيل المباشر في الدورة.",
      applicantTypeLabel: "نوع المتقدم",
      applicantIndividual: "فرد",
      applicantInstitution: "مؤسسة تعليمية",
      modeLabel: "نوع التدريب المفضل",
      modeCombined: "مسجّل وبث مباشر",
      modeCombinedSub: "متزامن — مسجّل أو مباشر",
      modeGroupInPerson: "وجاهي جماعي",
      modeGroupInPersonSub: "حضوري في الموقع",
      modePrivate: "خاص ١:١",
      modePrivateSub: "تدريب مكثف شخصي",
      nameLabel: "الاسم الكامل",
      namePlaceholder: "محمد عبدالله",
      phoneLabel: "رقم الهاتف",
      emailLabel: "البريد الإلكتروني",
      interestLabel: "أنا مهتم بـ",
      interestPlaceholder: "اختر الفئة",
      programLabel: "البرنامج المطلوب",
      programPlaceholder: "اختر البرنامج",
      reasonLabel: "لماذا تريد الانضمام؟ (اختياري)",
      reasonPlaceholder: "شاركنا ما تطمح لتحقيقه...",
      youtubeLabel: "رابط خطاب سابق على يوتيوب (اختياري)",
      youtubePlaceholder: "https://youtube.com/...",
      orgNameLabel: "اسم المؤسسة التعليمية",
      orgNamePlaceholder: "مدرسة / جامعة / مركز",
      contactPersonLabel: "اسم المسؤول للتواصل",
      contactPersonPlaceholder: "الاسم الكامل",
      studentCountLabel: "عدد الطلاب المتوقع",
      teacherCountLabel: "عدد المعلمين للتأهيل",
      workbookCountLabel: "عدد الكراسات المطلوبة (اختياري)",
      orgMessageLabel: "رسالة إضافية",
      orgMessagePlaceholder: "أخبرنا عن احتياجات مؤسستكم...",
      submitBtn: "تأكيد الطلب",
      submitting: "جاري الإرسال...",
      sidePanelTitle: "صوتك أداة.\nغيّر بها مسارك.",
      sidePanelText: "كل إنجاز كبير بدأ بخطوة واحدة. فريقنا هنا ليرافقك في رحلة تحويل أسلوبك وصقل تأثيرك.",
      quickQ: "استفسارات سريعة؟",
      sidePanelStep1: "التقديم",
      sidePanelStep1Desc: "أرسل طلبك الآن",
      sidePanelStep2: "التوجيه",
      sidePanelStep2Desc: "نؤكد البرنامج المختار",
      sidePanelStep3: "الانطلاق",
      sidePanelStep3Desc: "تبدأ رحلتك التحويلية",
      sideQuote: "لا تُقاس الكلمات بعددها، بل بالأثر الذي تتركه في القلوب.",
      successTitle: "تم استلام طلبك بنجاح!",
      successDesc: "سنتواصل معك قريباً لتأكيد تفاصيل الانضمام.",
      interestOptions: [
        { value: "adult", label: "نفسي (شاب / مهني)" },
        { value: "trainer", label: "اعتماد المدربين (ToT)" },
        { value: "parent", label: "برنامج المعلمين وأولياء الأمور" },
        { value: "school", label: "برنامج الأطفال (عبر المدرسة)" },
      ],
      featureGroupOnline: "تدريب جماعي عن بعد — مرونة تامة",
      featureGroupInPerson: "تدريب جماعي وجاهي — تفاعل مباشر",
      featurePrivate: "تدريب خاص ١:١ — وجاهي أو عن بعد",
      featureLang: "عربية، إنجليزية، وفرنسية",
      wizard: {
        stepLabel: "خطوة",
        of: "من",
        next: "التالي",
        back: "السابق",
        submit: "إرسال الطلب",
        submitting: "جاري الإرسال...",
        skip: "تخطي",
        optional: "اختياري",
        // Step 1
        step1Title: "من أنت؟",
        step1Sub: "اختر ما يصفك لإكمال طلب التسجيل",
        audIndividual: "فرد",
        audIndividualDesc: "أريد تطوير نفسي",
        audTeacher: "معلم",
        audTeacherDesc: "أعلّم وأريد التميّز",
        audParent: "ولي أمر",
        audParentDesc: "أبني مهارات أبنائي",
        audInstitution: "مؤسسة",
        audInstitutionDesc: "مدرسة أو مركز",
        // Step 2
        step2Title: "ما هدفك؟",
        step2Sub: "اختر هدفك الأهم — يمكنك إضافة تفاصيل أكثر لاحقاً",
        goalConfidence: "أبني ثقتي بنفسي",
        goalSpeak: "أتحدث أمام الجمهور بثبات",
        goalCareer: "أطوّر مساري المهني",
        goalTrain: "أصبح مدرباً معتمداً",
        goalTeach: "أعلّم الأطفال الخطابة",
        goalChild: "أطوّر مهارات طفلي",
        goalOther: "هدف آخر",
        goalOtherPlaceholder: "اكتب هدفك بإيجاز...",
        // Step 3
        step3Title: "اختر برنامجك",
        step3Sub: "اختر البرنامج الذي ترغب بالتسجيل فيه",
        // Step 4
        step4Title: "بيانات التواصل",
        step4Sub: "آخر خطوة — سنتواصل معك خلال 24 ساعة",
        msgLabel: "رسالة إضافية (اختياري)",
        msgPlaceholder: "أي تفاصيل إضافية تودّ مشاركتها...",
        // Validation
        errPickAudience: "يرجى اختيار من أنت",
        errPickGoal: "يرجى اختيار هدفك",
        errPickProgram: "يرجى اختيار البرنامج الذي ترغب بالتسجيل فيه",
        errFillContact: "يرجى تعبئة الاسم والهاتف والبريد",
        errInstitutionName: "يرجى إدخال اسم المؤسسة",
        // Summary chips on step 4
        summaryFor: "لـ",
        summaryGoal: "الهدف",
        summaryProgram: "البرنامج",
        // Privacy / accessibility
        privacyNote: "ببياناتك ستبقى محفوظة لدينا فقط — لن نشاركها مع أي طرف ثالث، ولن نرسل لك رسائل ترويجية بدون إذن.",
        privacyLinkText: "اطّلع على سياسة الخصوصية",
        ariaName: "الاسم الكامل",
        ariaPhone: "رقم الهاتف",
        ariaEmail: "البريد الإلكتروني",
        ariaOrg: "اسم المؤسسة",
        ariaStudents: "عدد الطلاب المتوقع",
        ariaTeachers: "عدد المعلمين للتأهيل",
        ariaMessage: "رسالة إضافية (اختياري)",
      },
    },
    footer: {
      about: "مؤسسة تعليمية وتدريبية متخصصة في بناء مهارات الخطابة والتواصل للشباب والمهنيين والمعلمين.",
      programsHeading: "البرامج",
      linksHeading: "روابط سريعة",
      contactHeading: "تواصل معنا",
      whatsappLabel: "تواصل عبر واتساب",
      linkLabels: { structure: "البرامج التدريبية", wisdom: "مقتطفات", workbooks: "الكراسات", testimonials: "آراء العملاء" },
      terms: "الشروط والأحكام",
      privacy: "سياسة الخصوصية",
      copyright: `© ${new Date().getFullYear()} بكلمة. جميع الحقوق محفوظة.`,
    },
    gallery: {
      heading: "معرض بكلمة",
      sub: "لحظات حقيقية من دوراتنا التدريبية وخطاباتنا — أفواج متعددة، وجوه تغيّرت، وكلمات صنعت أثراً.",
      tabCohorts: "الأفواج التدريبية",
      tabSpeeches: "الخطابات",
      lightboxPrev: "السابقة",
      lightboxNext: "التالية",
      lightboxClose: "إغلاق",
    },
    videos: {
      heading: "من الميدان — فيديوهات",
      sub: "مكتبة تعليمية شاملة تُغطي كل مهارات التواصل والإلقاء والتفاوض — فيديوهات قصيرة بترجمة عربية.",
      learnLabel: "ما ستتعلمه",
      suhaibBadge: "صهيب الخوالدة",
      worldBadge: "خطاب عالمي",
      tabs: {
        all: "الكل",
        opening: "البداية",
        closing: "النهاية",
        storytelling: "السرد والقصة",
        humor: "الفكاهة",
        voice: "الصوت والإيقاع",
        body: "الحضور المسرحي",
      },
      skillLabel: "الاستفادة",
    },
    modal: {
      aboutHeading: "عن البرنامج",
      introVideoHeading: "فيديو تعريفي بالبرنامج",
      introVideoSoon: "سيتم إضافة الفيديو التعريفي قريباً",
      transformHeading: "التحول المستهدف",
      sessionsHeading: "الجلسات التدريبية",
      outcomesHeading: "مخرجات البرنامج",
      workbookHeading: "الكراسة المرافقة",
      audienceHeading: "الفئة المستهدفة",
      orderWorkbook: "طلب الكراسة",
      enrollBtn: "التسجيل في هذا البرنامج",
      buyRecordedBtn: "سجّل في الدورة",
      registerInterestBtn: "سجّل اهتمامك",
      hoursUnit: "ساعة تدريبية",
    },
    programPage: {
      breadcrumbPrograms: "البرامج",
      backToPrograms: "العودة للبرامج",
      sectionAudience: "لمن هذا البرنامج؟",
      sectionProblem: "المشكلة التي يحلّها",
      sectionMeta: "تفاصيل البرنامج",
      metaDuration: "المدة",
      metaSessions: "عدد الجلسات",
      metaPrice: "سعر الدورة المسجّلة",
      metaHoursUnit: "ساعة",
      metaSessionsUnit: "جلسة",
      metaSchoolsOnly: "للمدارس فقط",
      sectionOutcomes: "مخرجات البرنامج",
      sectionModules: "الجلسات والمحاور",
      sectionWorkbook: "الكراسة المرافقة",
      sectionWorkbookStructure: "بنية الكراسة",
      workbookSectionsLabel: "قسم تدريبي",
      workbookUnitsLabel: "وحدة رئيسية",
      workbookFactsIntro: "مبني على كراسة تدريبية مُحكمة، بهيكل واضح وتمارين مُتدرّجة:",
      sectionFaq: "أسئلة شائعة",
      shareBtn: "مشاركة",
      shareCopied: "تم نسخ الرابط",
      transformationLabel: "التحوّل",
      primaryCta: "احجز مقعدك الآن",
      secondaryCta: "لدي سؤال قبل الحجز",
      schoolPrimaryCta: "اطلب عرضًا للمدرسة",
      schoolNote: "هذا البرنامج يُقدَّم للمدارس والمؤسسات حصراً عبر مدرّبين معتمدين، وليس للشراء الفردي المباشر.",
      finalCtaHeading: "خطوتك القادمة",
      finalCtaSub: "خطوة واحدة تفصلك عن أن تبدأ رحلتك مع",
      notFound: "لم نعثر على هذا البرنامج",
      backHome: "العودة للرئيسية",
      reassure: "سيتم إنشاء حسابك أو تسجيل دخولك أولًا، ثم إكمال طلب التسجيل والدفع بأمان.",
      schoolMailSubject: "طلب عرض مدرسة لبرنامج",
      schoolMailBody: "السلام عليكم،%0D%0A%0D%0Aأودّ ترتيب البرنامج التالي لمدرستنا/مؤسستنا: ",
      schoolMailBody2: "%0D%0A%0D%0Aاسم المدرسة: %0D%0Aالمسؤول للتواصل: %0D%0Aعدد الطلاب: %0D%0Aرقم التواصل: %0D%0Aملاحظات: %0D%0A",
      questionMailSubject: "سؤال قبل الحجز — ",
      waBookingMsg: (title: string) => `السلام عليكم، لدي سؤال قبل الحجز في برنامج «${title}». أرجو التواصل معي.`,
      waSchoolMsg: (title: string) => `السلام عليكم، نودّ ترتيب برنامج «${title}» لمدرستنا. أرجو التواصل لمعرفة التفاصيل.`,
      shareTitleSuffix: " — بكلمة",
      shareSiteName: "بكلمة",
      includes: {
        heading: "ما الذي تحصل عليه فعلاً؟",
        sub: "التسجيل ليس شراء مقاطع مصوّرة. هو التحاق ببرنامج متكامل يرافقك حتى تقف أمام جمهور حقيقي.",
        items: [
          { title: "جلسات مسجّلة ومباشرة", body: "شاهد الجلسات وقت ما تشاء، والتحق باللقاءات المباشرة عبر Zoom لتتمرّن وتُسأل وتُصحَّح مباشرة." },
          { title: "كرّاسة تفاعلية داخل المنصّة", body: "اقرأ الكرّاسة صفحةً صفحة داخل الموقع، وسجّل ملاحظاتك على المقاطع التي تختارها — لا ملف تُنزّله وتنساه." },
          { title: "مكتبة عملية ومصادر", body: "نماذج خطابات، ومقاطع محلَّلة، وقوائم تحقّق، ومراجع مختارة لكل مهارة — تعود إليها كلما احتجت قبل خطبة." },
          { title: "واجبات يصحّحها مدرّب", body: "تمرين على كل محور تسلّمه فيقرأه مدرّب حقيقي ويردّ عليك بملاحظة مكتوبة، لا تصحيحاً آلياً." },
          { title: "خريطة مهارات تتقدّم معك", body: "ثماني مهارات خطابة تُقاس ونقاطها تنمو مع كل واجب مُجتاز، فترى أين تقوى وأين تحتاج عملاً." },
          { title: "مشروع تخرّج أمام جمهور", body: "تُنهي البرنامج بخطبة تلقيها أمام حضور حقيقي وتُقيَّم عليها — لأن المهارة لا تُثبت إلا على المنصّة." },
        ],
      },
      tabs: {
        overview: "نظرة عامة",
        audience: "لمن هذا البرنامج؟",
        outcomes: "ماذا ستتعلم؟",
        modules: "المحاور والمواد",
        workbook: "الكراسة والمواد",
        booking: "السعر والحجز",
        faq: "الأسئلة الشائعة",
      },
      bookingHeading: "السعر والحجز",
      bookingSub: "كل التفاصيل التي تحتاجها قبل أن تحجز مقعدك.",
      deliveryHeading: "صيغ الدراسة المتاحة",
      deliveryBody: "يمكن تقديم الدورة مسجّلة، مباشرة عبر Zoom، أو بصيغة هجينة تجمع بينهما. السعر الظاهر هنا للدورة المسجّلة؛ أما دفعات Zoom والصيغة الهجينة فتُؤكّد مواعيدها ورسومها عند التسجيل.",
    },
  },

  en: {
    nav: {
      structure: "Training Programs",
      wisdom: "Notes",
      workbooks: "Workbooks",
      testimonials: "Reviews",
      gallery: "Gallery",
      videos: "Videos",
      events: "In-Person Courses",
      courses: "Courses",
      cta: "Start Now",
      mobileCta: "Start Your Journey",
    },
    hero: {
      badge: "A Comprehensive Public Speaking & Influence Methodology",
      h1a: "Bikalima,",
      h1b: "We Create Impact.",
      sub: "A foundational public speaking course for youth and professionals, branching into tracks for certified trainers, educators, parents, and children.",
      ctaPrimary: "Start learning now",
      ctaSecondary: "Explore the Programs",
      trustStrip: {
        trainees: "800+ trainees",
        countries: "7 countries",
        rating: "4.9 / 5 rating",
        years: "6+ years of practice",
      },
      institutions: {
        label: "Bikalima or its founder has worked with",
        list: "Qatar Foundation · UAE Ministry of Economy · Aston University · Breakthrough T1D · TEDx",
      },
      compare: {
        heading: "Compare the programs",
        sub: "Four programs · one path · pick the one that fits you.",
        bestPick: "Best place to start",
        colProgram: "Program",
        colAudience: "Audience",
        colDuration: "Duration",
        colPrice: "Recorded price",
        colOutcome: "Expected outcome",
        colPrereq: "Prerequisite",
        none: "None",
        viewDetails: "View details",
        viewAll: "View all programs",
        priceSchools: "Schools only",
      },
      imageQuotes: [
        { text: "Don't fear your voice... fear the day you decide not to speak.", author: "Suhaib Al-Khawaldeh" },
        { text: "An honest word travels farther than a thousand polished speeches.", author: "Suhaib Al-Khawaldeh" },
        { text: "It's not about speaking... it's about leaving an impact.", author: "Suhaib Al-Khawaldeh" },
        { text: "Your voice is the most powerful tool for change you possess.", author: "Suhaib Al-Khawaldeh" },
        { text: "Confidence isn't born... it's built one word at a time.", author: "Suhaib Al-Khawaldeh" },
        { text: "Whoever owns the word, owns the influence.", author: "Suhaib Al-Khawaldeh" },
      ],
    },
    trainerBio: {
      heading: "About the Author & Trainer",
      name: "Suhaib Al-Khawaldeh",
      title: "Consultant · PhD Researcher · TEDx Speaker · Founder of Bikalima",
      bio: "A consultant working with a range of developmental, charitable, and governmental organizations in the region, including Qatar Foundation, the UAE Ministry of Economy and Tourism, and the funding organization Breakthrough T1D, among others — contributing to program design, partnership building, and supporting development and innovation initiatives.\n\nA lecturer at several universities, including the American University in the UAE and Aston University in the United Kingdom, in digital economics and entrepreneurship. He is a PhD researcher at Aston University (UK) and holds a Master's degree in Management from the University of Birmingham (UK).\n\nA TEDx speaker and author of the Bikalima training workbook series. He delivers training programs in the art of public speaking, professional leadership skills, and project funding — as an extension of his institutional work and impact experience. He has led dozens of programs and speeches across Jordan and the Arab world, training hundreds of participants of all ages and backgrounds.",
    },
    author: {
      sectionTitle: "A Message to Every Educator, Speaker, and Learner",
      cards: [
        { to: "To the Educator", text: "You don't just teach a subject — you shape voices. Every word you speak in front of your student leaves a mark on how they speak to the world. Our program was born from a deep belief that the greatest gift you can give a student is the conviction that their voice matters." },
        { to: "To the Speaker", text: "Your words are not just performance — they are presence. And presence doesn't come from technique alone, but from being connected to what you truly believe. Bikalima doesn't just teach you how to speak; it brings you back to why you speak, and gives you the tools to convey your soul before your voice." },
        { to: "To the Beginner", text: "It doesn't matter where you start. What matters is that you start. 70% of people suffer from fear of public speaking — but real impact is made by the 30% who decided to learn. You're here — and that's the first and hardest decision." },
      ],
    },
    why: {
      heading: "Why Bikalima?",
      sub: "The problem isn't that you have nothing to say — it's that what you say doesn't land the way you intend.",
      items: [
        { title: "Because your silence in the critical moment has a cost", desc: "A missed job because the interview didn't convince. A brilliant idea lost because no one heard it. A partnership that never formed because the words weren't there. This isn't a communication problem — it's a real loss that can be prevented." },
        { title: "Because the problem isn't in your ideas", desc: "Most people who struggle with communication have deep, genuine thoughts. The problem is the gap between what they think and what reaches others. Bikalima bridges that gap with a methodology that works on expression, mindset, and delivery together." },
        { title: "Because confidence is a skill, not a talent", desc: "70% of those who go silent weren't born afraid — they learned fear somewhere along the way. We work on the roots before the performance. What was built can be rebuilt — and that is exactly what we do." },
      ],
      consultIntro: {
        heading: "Book Your Free Consultation",
        what: "A 20-minute one-on-one session with Suhaib Al-Khawaldeh — to assess your current level and identify the best path forward.",
        audiences: [
          { icon: "🎓", label: "Students & Graduates", desc: "Stand out in interviews and build your professional voice" },
          { icon: "💼", label: "Professionals & Employees", desc: "Lead meetings and make your voice heard in the organization" },
          { icon: "🏫", label: "Teachers & Trainers", desc: "A delivery style that inspires, persuades, and impacts the classroom" },
          { icon: "👨‍👩‍👧", label: "Parents & Guardians", desc: "Build your child's confidence in self-expression from an early age" },
        ],
      },
    },
    structure: {
      heading: "Training Programs",
      sub: "Recorded, live on Zoom, or blended courses — upcoming in-person dates are listed separately below.",
      coreBadge: "Core Flagship Course",
      coreSubtitle: "Crafting Influence & the Art of Public Speaking",
      hoursUnit: "hours",
      sessionsUnit: "sessions",
      viewDetails: "Explore the Program",
      viewProgramPage: "Explore the Program",
      prereqLabels: { tot: "Required: The Influential Speaker", teachers: "Recommended: The Influential Speaker", children: "Via Educators Program Graduates" },
      recordedLabel: "Recorded",
      recordedDesc: "Study at your own pace",
      zoomLabel: "Live on Zoom",
      blendedLabel: "Recorded + Zoom",
      deliveryOptions: "Recorded · Zoom · Blended",
      recordedPriceLabel: "Recorded course price",
      inPersonLabel: "In-Person",
      privateLabel: "Private Training 1:1",
      privateDesc: "On demand — schedule and price determined later",
      contactForPrivate: "Contact Us",
      upcomingEventsHeading: "In-Person Courses",
      upcomingEventsSub: "Upcoming classroom training dates — review the details and register here",
      noUpcomingEvents: "No in-person courses are currently announced — follow us for new dates",
      countriesHeading: "Countries where we offer in-person courses",
      countries: { uae: "UAE", saudi: "Saudi Arabia", jordan: "Jordan", uk: "United Kingdom" },
      registerNow: "Register Now",
      spotsLeft: "spots left",
      enrollNow: "Reserve Your Spot",
      priceUnit: "JOD",
      rating: "4.9",
      startHelpHeading: "Need a consultation before you begin?",
      startHelpSub: "Tell us your goal and we'll help you pick the right program for your journey.",
      startHelpBtn: "Help me choose a program",
      startHelpContact: "Contact Us",
    },
    wisdom: {
      badge: "Notes from Our Publications",
      heading: "Bikalima's Philosophy",
      sub: "What you find in our publications isn't just exercises — it's an educational vision that believes the word is a tool for building human beings.",
    },
    workbooks: {
      heading: "Training Workbooks",
      sub: "Carefully designed training workbooks — available independently or as part of a program.",
      pricesIn: "Prices in",
      priceLabel: "Price",
      orderBtn: "Buy Workbook",
      orderBtnKit: "Buy Training Kit",
      companion: "Target Audience",
      previewTitle: "Page Preview",
      previewPage: "Page",
      samplePdfBtn: "Download Sample PDF",
      samplePdfNote: "Download a free sample of the workbook to preview the content",
      orderTitle: "Order Workbook",
      quantityLabel: "Number of Copies",
      formatLabel: "Format",
      formatPdf: "Digital Copy (PDF)",
      formatPrint: "Printed Copy",
      deliveryLabel: "Delivery Address",
      deliveryPlaceholder: "City, street, building number...",
      deliveryNote: "Price does not include delivery — delivery cost will be confirmed later.",
      totalLabel: "Total",
      submitOrder: "Confirm Order",
      submittingOrder: "Submitting...",
      orderSuccess: "Order received! We will contact you soon.",
      nameLabel: "Full Name",
      namePlaceholder: "Your full name",
      phoneLabel: "Phone Number",
      emailLabel: "Email",
    },
    testimonials: { heading: "Client Reviews" },
    testimonialsSection: {
      eyebrow: "Client Reviews",
      heading: "Words from those who came before you",
      sub: "Real voices from trainees, trainers, and parents — they started their journey with us. Why not be next?",
    },
    stats: {
      eyebrow: "Bikalima by the Numbers",
      heading: "Impact that keeps growing",
      sub: "Numbers that tell the story of thousands of moments where silence turned into voice.",
      trainees: "Trainees",
      countries: "Countries worldwide",
      programs: "Training programs",
      trainers: "Certified trainers",
    },
    beforeAfter: {
      eyebrow: "Before & After",
      heading: "Real transformation starts in the first session",
      sub: "Not just a course — a journey that reshapes your relationship with your voice, ideas, and presence.",
      beforeLabel: "Before Bikalima",
      beforeHeading: "You before you start",
      beforeItems: [
        "Your ideas are clear in your head but scatter when you speak.",
        "Your hands and voice tremble in front of any audience.",
        "You lose career opportunities to interviews that didn't convince.",
        "You avoid sharing your opinion in meetings and discussions.",
        "You think public speaking is an innate talent, not a skill.",
      ],
      transformLabel: "The Transformation",
      afterLabel: "After Bikalima",
      afterHeading: "You after the program",
      afterItems: [
        "You deliver your idea in seconds with clarity and confidence.",
        "You walk into any room knowing your voice deserves to be heard.",
        "You convince in interviews and lead meetings with presence.",
        "You give speeches that leave a lasting impact.",
        "You own public speaking as a skill you can keep building.",
      ],
    },
    journey: {
      eyebrow: "The Student Journey",
      heading: "How does your journey with Bikalima begin?",
      sub: "Six clear steps from the moment you're interested to the moment you receive your certificate.",
      steps: [
        { title: "Register your interest", desc: "Fill out the registration form or book a free consultation." },
        { title: "You choose your program", desc: "Review the program details and choose the path you want to join." },
        { title: "You start training", desc: "Attend in-person or online sessions with the trainer." },
        { title: "You practice", desc: "Real performance exercises with personal feedback." },
        { title: "You get evaluated", desc: "Professional assessment of your speech with growth recommendations." },
        { title: "You receive your certificate", desc: "An accredited Bikalima certificate that confirms your achievement." },
      ],
    },
    journeyCta: {
      heading: "Start your journey with Bikalima today",
      sub: "One step away from making your voice your most powerful tool.",
      button: "Start your journey",
    },
    faq: {
      heading: "Frequently Asked Questions",
      sub: "Everything you need to know about our programs and tracks.",
      items: [
        { q: "Which course should I start with?", a: "The core course \"The Influential Speaker\" is the natural entry point. If you want to become a certified trainer, you'll need to complete it first. If you're an educator or parent, you can join our specialized program directly." },
        { q: "What's the difference between group training and private training?", a: "Group training lets you participate within a group setting, either online or in-person. Private 1:1 training is a fully personalized, intensive experience (in-person or remote), with a faster pace and immediate in-depth feedback." },
        { q: "Is the core course required before the Educators & Parents program?", a: "No, it's not required. However, completing the core course maximizes the benefit and gives you the trainee experience before becoming a guide." },
        { q: "How does the children's program reach schools?", a: "The 'Young Speaker' program is delivered to schools exclusively through certified graduates of Bikalima's Educators & Parents program." },
        { q: "Can I purchase a publication without enrolling in a program?", a: "Yes! You can purchase any publication independently. The publications are designed to be useful on their own, but maximum benefit comes with the accompanying program." },
        { q: "Are the programs in-person or online?", a: "We offer multiple formats: group online training, group in-person training, and private 1:1 training (in-person or online)." },
        { q: "Do you offer customized training for educational institutions?", a: "Yes! We provide tailored programs for educational institutions to train their teachers and integrate public speaking into their extracurricular programs. Contact us through the institutions form on the registration page." },
        { q: "How long are the training programs?", a: "Duration varies by program: The Influential Speaker is 27 hours (14 sessions), The Certified Trainer is 40 hours (20 sessions), Educators & Parents is 21 hours (11 sessions), and The Young Speaker is 18 hours (9 sessions)." },
        { q: "Are there discounts available?", a: "Yes, we offer group discounts and early registration discounts. Contact us directly and we'll share available pricing and offers." },
        { q: "Can I share a link to a previous speech during registration?", a: "Absolutely! We encourage you to share a YouTube link to any previous speech or presentation. This helps us understand your current level and provide a more personalized training experience." },
      ],
    },
    enroll: {
      heading: "For Institutions & Groups",
      sub: "Request a custom proposal for your institution or group. For individuals: use the “Start learning now” button above to enroll directly in a course.",
      applicantTypeLabel: "Applicant Type",
      applicantIndividual: "Individual",
      applicantInstitution: "Educational Institution",
      modeLabel: "Preferred Training Format",
      modeCombined: "Recorded & Live",
      modeCombinedSub: "Sync — recorded or live",
      modeGroupInPerson: "In-Person",
      modeGroupInPersonSub: "Attend in person",
      modePrivate: "Private 1:1",
      modePrivateSub: "Intensive personal",
      nameLabel: "Full Name",
      namePlaceholder: "John Smith",
      phoneLabel: "Phone Number",
      emailLabel: "Email Address",
      interestLabel: "I am interested in",
      interestPlaceholder: "Select category",
      programLabel: "Requested Program",
      programPlaceholder: "Select program",
      reasonLabel: "Why do you want to join? (optional)",
      reasonPlaceholder: "Tell us what you hope to achieve...",
      youtubeLabel: "Link to a previous speech on YouTube (optional)",
      youtubePlaceholder: "https://youtube.com/...",
      orgNameLabel: "Institution Name",
      orgNamePlaceholder: "School / University / Center",
      contactPersonLabel: "Contact Person",
      contactPersonPlaceholder: "Full name",
      studentCountLabel: "Expected Number of Students",
      teacherCountLabel: "Number of Teachers to Train",
      workbookCountLabel: "Number of Publications Needed (optional)",
      orgMessageLabel: "Additional Message",
      orgMessagePlaceholder: "Tell us about your institution's needs...",
      submitBtn: "Confirm Registration",
      submitting: "Sending...",
      sidePanelTitle: "Your Voice Is a Tool.\nUse It to Change Everything.",
      sidePanelText: "Every great achievement began with a single step. Our team is here to guide your transformation — from the first word to lasting impact.",
      quickQ: "Quick questions?",
      sidePanelStep1: "Apply",
      sidePanelStep1Desc: "Submit your request now",
      sidePanelStep2: "Guidance",
      sidePanelStep2Desc: "We confirm your selected program",
      sidePanelStep3: "Launch",
      sidePanelStep3Desc: "Your transformation begins",
      sideQuote: "Words are not measured by their count, but by the impression they leave on hearts.",
      successTitle: "Your request has been received!",
      successDesc: "We will contact you shortly to confirm your enrollment details.",
      interestOptions: [
        { value: "adult", label: "For myself (youth / professional)" },
        { value: "trainer", label: "Certified Trainer (ToT)" },
        { value: "parent", label: "Educators & Parents Program" },
        { value: "school", label: "Children's Program (via school)" },
      ],
      featureGroupOnline: "Group online training — full flexibility",
      featureGroupInPerson: "Group in-person training — direct interaction",
      featurePrivate: "Private 1:1 — in-person or online",
      featureLang: "Arabic, English, and French",
      wizard: {
        stepLabel: "Step",
        of: "of",
        next: "Next",
        back: "Back",
        submit: "Submit Request",
        submitting: "Sending...",
        skip: "Skip",
        optional: "optional",
        // Step 1
        step1Title: "Who are you?",
        step1Sub: "Pick what describes you to continue your registration",
        audIndividual: "Individual",
        audIndividualDesc: "I want to develop myself",
        audTeacher: "Teacher",
        audTeacherDesc: "I teach and want to excel",
        audParent: "Parent",
        audParentDesc: "I'm building my child's skills",
        audInstitution: "Institution",
        audInstitutionDesc: "School or center",
        // Step 2
        step2Title: "What is your goal?",
        step2Sub: "Pick your top goal — you can add more details later",
        goalConfidence: "Build my self-confidence",
        goalSpeak: "Speak in front of audiences",
        goalCareer: "Advance my career",
        goalTrain: "Become a certified trainer",
        goalTeach: "Teach children public speaking",
        goalChild: "Develop my child's skills",
        goalOther: "Other goal",
        goalOtherPlaceholder: "Briefly describe your goal...",
        // Step 3
        step3Title: "Choose your program",
        step3Sub: "Choose the program you want to join",
        // Step 4
        step4Title: "Contact details",
        step4Sub: "Last step — we'll reach out within 24 hours",
        msgLabel: "Additional message (optional)",
        msgPlaceholder: "Any additional details you'd like to share...",
        // Validation
        errPickAudience: "Please select who you are",
        errPickGoal: "Please choose your goal",
        errPickProgram: "Please choose the program you want to join",
        errFillContact: "Please fill in name, phone and email",
        errInstitutionName: "Please enter the institution name",
        // Summary chips on step 4
        summaryFor: "For",
        summaryGoal: "Goal",
        summaryProgram: "Program",
        // Privacy / accessibility
        privacyNote: "Your data is kept private — we will never share it with any third party or send promotional messages without your consent.",
        privacyLinkText: "Read the privacy policy",
        ariaName: "Full name",
        ariaPhone: "Phone number",
        ariaEmail: "Email address",
        ariaOrg: "Institution name",
        ariaStudents: "Expected number of students",
        ariaTeachers: "Number of teachers to train",
        ariaMessage: "Additional message (optional)",
      },
    },
    footer: {
      about: "An educational and training institution specializing in building public speaking and communication skills for youth, professionals, and educators.",
      programsHeading: "Programs",
      linksHeading: "Quick Links",
      contactHeading: "Contact Us",
      whatsappLabel: "Chat on WhatsApp",
      linkLabels: { structure: "Training Programs", wisdom: "Notes", workbooks: "Workbooks", testimonials: "Reviews" },
      terms: "Terms & Conditions",
      privacy: "Privacy Policy",
      copyright: `© ${new Date().getFullYear()} Bikalima. All rights reserved.`,
    },
    gallery: {
      heading: "Bikalima Gallery",
      sub: "Real moments from our training programs and speeches — cohorts across the globe, lives changed, words that made an impact.",
      tabCohorts: "Training Cohorts",
      tabSpeeches: "Speeches",
      lightboxPrev: "Previous",
      lightboxNext: "Next",
      lightboxClose: "Close",
    },
    videos: {
      heading: "From the Field — Videos",
      sub: "A comprehensive learning library covering every communication, speaking and negotiation skill — short videos with Arabic subtitles.",
      learnLabel: "What you'll learn",
      suhaibBadge: "Suhaib Al-Khawaldeh",
      worldBadge: "World Speech",
      tabs: {
        all: "All",
        opening: "Opening",
        closing: "Closing",
        storytelling: "Storytelling",
        humor: "Humor",
        voice: "Voice & Rhythm",
        body: "Stage Presence",
      },
      skillLabel: "Takeaway",
    },
    modal: {
      aboutHeading: "About the Program",
      introVideoHeading: "Program Introduction Video",
      introVideoSoon: "Introduction video coming soon",
      transformHeading: "Targeted Transformation",
      sessionsHeading: "Training Sessions",
      outcomesHeading: "Program Outcomes",
      workbookHeading: "Companion Publication",
      audienceHeading: "Target Audience",
      orderWorkbook: "Order Publication",
      enrollBtn: "Enroll in this Program",
      buyRecordedBtn: "Enroll Now",
      registerInterestBtn: "Register Interest",
      hoursUnit: "training hours",
    },
    programPage: {
      breadcrumbPrograms: "Programs",
      backToPrograms: "Back to Programs",
      sectionAudience: "Who Is This Program For?",
      sectionProblem: "The Problem It Solves",
      sectionMeta: "Program Details",
      metaDuration: "Duration",
      metaSessions: "Sessions",
      metaPrice: "Recorded course price",
      metaHoursUnit: "hours",
      metaSessionsUnit: "sessions",
      metaSchoolsOnly: "Schools only",
      sectionOutcomes: "Program Outcomes",
      sectionModules: "Sessions & Modules",
      sectionWorkbook: "The Companion Workbook",
      sectionWorkbookStructure: "Workbook Structure",
      workbookSectionsLabel: "training sections",
      workbookUnitsLabel: "core units",
      workbookFactsIntro: "Built on a rigorously structured training workbook with progressive exercises:",
      sectionFaq: "Frequently Asked Questions",
      shareBtn: "Share",
      shareCopied: "Link copied",
      transformationLabel: "Transformation",
      primaryCta: "Reserve Your Seat",
      secondaryCta: "I have a question",
      schoolPrimaryCta: "Request a school proposal",
      schoolNote: "This program is delivered exclusively to schools and institutions through certified trainers — not as an individual purchase.",
      finalCtaHeading: "Your Next Step",
      finalCtaSub: "One step away from starting your journey with",
      notFound: "Program not found",
      backHome: "Back to Home",
      reassure: "We'll create your account or sign you in first, then complete enrollment and payment securely.",
      schoolMailSubject: "School proposal request for program",
      schoolMailBody: "Hello,%0D%0A%0D%0AWe'd like to arrange the following program for our school/institution: ",
      schoolMailBody2: "%0D%0A%0D%0ASchool name: %0D%0AContact person: %0D%0AStudent count: %0D%0APhone: %0D%0ANotes: %0D%0A",
      questionMailSubject: "Question before booking — ",
      waBookingMsg: (title: string) => `Hello, I have a question before booking the "${title}" program. Please get in touch.`,
      waSchoolMsg: (title: string) => `Hello, we'd like to arrange the "${title}" program for our school. Please get in touch with the details.`,
      shareTitleSuffix: " — Biklima",
      shareSiteName: "Biklima",
      includes: {
        heading: "What you actually get",
        sub: "Enrolling is not buying videos. It is joining a programme that stays with you until you stand in front of a real audience.",
        items: [
          { title: "Recorded and live sessions", body: "Watch on your own time, and join the live Zoom meetings to practise, ask, and be corrected on the spot." },
          { title: "An interactive workbook in the platform", body: "Read it page by page on the site and write notes against the passages you choose — not a file you download and forget." },
          { title: "A practical library and resources", body: "Model speeches, analysed clips, checklists and selected references for every skill — there whenever a talk is coming." },
          { title: "Assignments a trainer grades", body: "An exercise on each module, read by a real trainer who writes back to you. No automated marking." },
          { title: "A skills map that grows with you", body: "Eight speaking skills are scored, and the points rise with every assignment you pass, so you can see where you are strong." },
          { title: "A graduation project before an audience", body: "You finish by delivering a speech to a real audience and being assessed on it — a skill is only proven on the stage." },
        ],
      },
      tabs: {
        overview: "Overview",
        audience: "Who It's For",
        outcomes: "What You'll Learn",
        modules: "Modules & Materials",
        workbook: "Workbook & Materials",
        booking: "Price & Booking",
        faq: "FAQ",
      },
      bookingHeading: "Price & Booking",
      bookingSub: "Everything you need before reserving your seat.",
      deliveryHeading: "Available learning formats",
      deliveryBody: "The course may be delivered as recorded learning, live on Zoom, or a blended combination. The displayed price is for the recorded course; Zoom and blended cohort dates and fees are confirmed during registration.",
    },
  },


} as const;

export type Translations = typeof T[Lang];
