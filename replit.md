...
- **Pages**: 
  ...
  - `src/components/speech-evaluation-form.tsx` — bilingual (ar/en) "تقييم خطابك خلال 60 ثانية" lead-gen form with two modes (text / video URL), captures fullName + email + WhatsApp; posts to `POST /api/speech-evaluation` (rate-limited 5/hr/IP); success state shows "سيصلك تقييم أولي من فريق بكلمة"; section anchor `#speech-evaluation`. Admin manages requests at `/admin/speech-evaluations` with statuses: جديد / قيد التقييم / تم الإرسال / تحوّل إلى طالب / ملغى (stored in `speech_evaluations` table with `lead_source = home_60sec_evaluation`).
